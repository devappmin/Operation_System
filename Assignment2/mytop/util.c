...
void getTTY(char path[PATH_LEN], char tty[TTY_LEN])
{
    ...
    { //fd 0이 없을 경우
        ...
        for (int i = 0; i <= STAT_TTY_NR_IDX; i++)
        { // 7행까지 read해 TTY_NR 획득
        ...
        {                                    // /dev 디렉터리 탐색
            ...
            {                                //문자 디바이스 파일의 디바이스 ID가 ttyNr과 같은 경우
...
void add_proc_list(char path[PATH_LEN], bool isPPS, bool aOption, bool uOption, bool xOption, unsigned long cpuTimeTable[PID_MAX])
{
    ...
    long double cpu = 0;
    bool update = false;
    if (!isPPS && cpuTimeTable != NULL)
    { //ttop일 경우
        if (cpuTimeTable[proc.pid] != 0)
        { //이전에 실행된 내역이 있을 경우 process 시작 시각 이후의 값이 아닌 이전 실행 이후의 값 불러옴
            update = true;
...
void search_proc(bool isPPS, bool aOption, bool uOption, bool xOption, unsigned long cpuTimeTable[PID_MAX])
{
    ...
    { // /proc 디렉터리 내 하위 파일들 탐색 시작
        ...
        for (int i = 0; i < len; i++)
        { //디렉터리가 PID인지 찾기
            if (!isdigit(dentry->d_name[i]))
            { //디렉터리명 중 숫자 아닌 문자가 있을 경우
                isPid = false;
                break;
            }
        }
        ...
        if (isPPS && !aOption)           //aOption이 없을 경우 자기 자신의 process만 보여줌
            if (statbuf.st_uid != myUid) //uid가 자기 자신과 다를 경우 skip
                continue;
        if (isPPS && !xOption)
        { //xOption이 없을 경우 nonTerminal process는 생략함
            char tty[TTY_LEN];
            memset(tty, '\0', TTY_LEN);
        ...
        if (isPPS && !aOption && !uOption && !xOption)
        { //모든 Option 없을 경우
            char tty[TTY_LEN];
            memset(tty, '\0', TTY_LEN);
            getTTY(path, tty);      //TTY 획득
